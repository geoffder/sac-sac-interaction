...
def setDends():
	...
	if 1:
		for i in xrange(int(h.numSecs)-1):
...
def dirBar(angle):
	...
	for k in xrange(len(xLocs)):
		dirXlocs[k], dirYlocs[k] = rotate(origin, (xLocs[k], yLocs[k]), math.radians(angle))
	for i in xrange(len(allBips)):
...
def tuning():
	...
	pkPN_iCa = np.divide(np.subtract(aPeaks_iCa,bPeaks_iCa),np.add(aPeaks_iCa,bPeaks_iCa))
	...
	for i in xrange(len(velRecs[0][0][0])): 
	...
	newFile = open(basest + fname,'w')
	for i in xrange(len(velRecs_iCa[0][0][0])): 
...
def dirTuning():
	...
	bThrAreas = range(8)
	#dirRecs = range(8)
	dirRecs = [range(8) for i in xrange(numTrials)]
	dirRecs_iCa = [range(8) for i in xrange(numTrials)]
	aPeaks_iCa = np.zeros(8)
	bPeaks_iCa = np.zeros(8)
	
	for k in xrange(numTrials):
		for i in xrange(len(dirLabel)):
			for j in xrange(len(xLocs)):
				dirXlocs[j], dirYlocs[j] = rotate((xOrigin,yOrigin), (xLocs[j],yLocs[j]), math.radians(dirLabel[i]))
			# model run
			setNoise()
			bar(speed, dirXlocs)
			# data stuff
	...
	fname = 'vmDirRecs.dat'
	newFile = open(basest + fname,'w')
	for i in xrange(len(dirRecs[0][0][0])): 
	...
	newFile = open(basest + fname,'w')
	for i in xrange(len(dirRecs_iCa[0][0][0])): 
